...
type Action =
  ...
  | { type: "SET_SYMPTOM_TEXT"; text: string }
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "SET_SYMPTOM_TEXT":
      return { ...state, symptomText: action.text };
    case "BOOKING_PENDING":
      return { ...state, submitting: true, error: null };
    case "BOOKING_CONFIRMED":
      return { ...state, submitting: false, step: "confirmed" };
    case "BOOKING_FAILED":
      return { ...state, submitting: false, error: action.error };
    case "HOLD_EXPIRED":
      return {
...
export function BookingFlow({ doctorId }: { doctorId: string }) {
  ...
  // Countdown that resets the flow back to slot selection once the hold expires,
  // so the patient can't try to book against a slot Postgres would reject anyway.
  useEffect(() => {
  ...
  async function handleConfirmBooking() {
    ...
    try {
      const res = await fetch("/api/appointments/book", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ holdId: state.holdId, symptomText: state.symptomText || undefined }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Failed to confirm booking");
      dispatch({ type: "BOOKING_CONFIRMED" });
    } catch (err) {
      dispatch({ type: "BOOKING_FAILED", error: err instanceof Error ? err.message : "Failed to confirm booking" });
    }
  }
    ...
    return (
      <Card>
        <CardBody className="space-y-4">
          ...
          <FormField label="Describe your symptoms (optional)" htmlFor="symptomText">
            <Textarea
              id="symptomText"
              rows={4}
              placeholder="e.g. Persistent headache for 3 days, mild fever..."
              value={state.symptomText}
              onChange={(e) => dispatch({ type: "SET_SYMPTOM_TEXT", text: e.target.value })}
            />
          </FormField>
          <div className="flex gap-2">
            <Button onClick={handleConfirmBooking} disabled={state.submitting}>
